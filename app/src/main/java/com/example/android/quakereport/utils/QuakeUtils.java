...
import com.example.android.quakereport.TonicQuake;
...
//            "{\"type\":\"Feature\",\"properties\":{\"mag\":6,\"place\":\"Pacific-Antarctic Ridge\",\"time\":1451986454620,\"updated\":1459202978040,\"tz\":-540,\"url\":\"http://earthquake.usgs.gov/earthquakes/eventpage/us10004bgk\",\"detail\":\"http://earthquake.usgs.gov/fdsnws/event/1/query?eventid=us10004bgk&format=geojson\",\"felt\":0,\"cdi\":1,\"mmi\":0,\"alert\":\"green\",\"status\":\"reviewed\",\"tsunami\":0,\"sig\":554,\"net\":\"us\",\"code\":\"10004bgk\",\"ids\":\",us10004bgk,gcmt20160105093415,\",\"sources\":\",us,gcmt,\",\"types\":\",cap,dyfi,geoserve,losspager,moment-tensor,nearby-cities,origin,phase-data,shakemap,\",\"nst\":null,\"dmin\":30.75,\"rms\":0.67,\"gap\":71,\"magType\":\"mww\",\"type\":\"earthquake\",\"title\":\"M 6.0 - Pacific-Antarctic Ridge\"},\"geometry\":{\"type\":\"Point\",\"coordinates\":[-136.2603,-54.2906,10]},\"id\":\"us10004bgk\"}],\"bbox\":[-153.4051,-54.2906,10,158.5463,59.6363,582.56]}";
    ...
    /**
     * Return a list of {@link TonicQuake} objects that has been built up from
     * parsing a JSON response.
     */
    public static List<TonicQuake> extractEarthquakes() {

        // Create an empty ArrayList that we can start adding earthquakes to
        List<TonicQuake> earthquakes = new ArrayList<TonicQuake>();

         String jsonResponse = NetworkUtils.run();
        ...
        try {
            ...
            for(int i = 0; i < features.length(); i++){
                ...
                JSONObject jsonQuake = features.getJSONObject(i);
                JSONObject quakeData = jsonQuake.getJSONObject("properties");
                String place = quakeData.getString("place");
                String date = formartDate(quakeData.getString("time"));
                Double mag = quakeData.getDouble("mag");

                 quake = new TonicQuake();
                 quake.setDate(date);
                 quake.setPlace(extractCountry(place));
                 quake.setTown(extractTown(place));
                 quake.setMagnitude(formattedMag(mag));
    ...
    private static String extractTown(String place) {
        if(place.indexOf("of") > -1){
            return place.substring(0, place.lastIndexOf("of"));
        } else {
            return  null;
        }
    }
    ...
    private static String extractCountry(String place) {
        if(place.indexOf("of") > -1){
            return place.substring(place.lastIndexOf("of")+3);
        } else {
            return  place;
        }
    }

    private static String formattedMag(Double mag) {
        DecimalFormat formatter = new DecimalFormat("0.0");
        String formattedMag = formatter.format(mag);
        return formattedMag;
    }

    private static String formartDate(String time) {
        Date date = new Date(Long.parseLong(time));
        SimpleDateFormat dateFormat = new SimpleDateFormat("LLL dd, yyyy");
        SimpleDateFormat timeFormat = new SimpleDateFormat("h:mm a");
        String stringDate = ""+dateFormat.format(date) + " at " + timeFormat.format(date);
        return stringDate;
    }
